...
class _TomorrowOrderState extends State<TomorrowOrder> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(locale.nextdayorder,style: TextStyle(
      ...
      body: Container(
        ...
        child: (!isLoading && newOrders != null && newOrders.length > 0)
          ...
          child: CircularProgressIndicator(),
            ...
            : Align(
          alignment: Alignment.center,
          child: Text(locale.noorder,textAlign: TextAlign.center,),
        ),
      ),
    );
  }
    ...
    return GestureDetector(
      onTap: () {
        if('${mainP.orderStatus}'.toUpperCase()=='CONFIRMED'){
          Navigator.pushNamed(context, PageRoutes.orderAcceptedPage,arguments: {
            'OrderDetail':mainP
          }).then((value){
            getOrderList();
          });
        }else if('${mainP.orderStatus}'.toUpperCase()=='OUT FOR DELIVERY'){
          Navigator.pushNamed(context, PageRoutes.signatureview,arguments: {
            'OrderDetail':mainP
          }).then((value){
            getOrderList();
          });
      ...
      child: Card(
        ...
        child: Column(
          children: [
            ...
            buildOrderInfoRow(context, '$apCurency ${mainP.remainingPrice}', '${mainP.paymentMethod}',
                '${mainP.orderStatus}'),
          ],
        ),
      ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 16),
                    (mainP.items!=null && mainP.items.length>0)?Text(locale.orderedOn + ' ${mainP.items[0].orderDate}',
                        style: Theme.of(context)
                            .textTheme
                            .subtitle2
                            .copyWith(fontSize: 10.5)):SizedBox.shrink(),
                  ],
                ),
              ),
            ],
